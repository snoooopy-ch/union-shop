                            ...
                            @if( count($store['locations']) >= 1 )
                                @foreach($store['locations'] as $location)
                                    <tr class="text-center">
                                        ...
                                        <x-table-column>
                                            <form action="{{ route('store.location.update', ['storeUuid'=> $store['uuid'], 'locationUuid' => $location['uuid'] ]) }}" method="POST">
                                                @csrf
                                                <input type="number" name="shopper_limit" value="{{ $location['shopper_limit'] ?? 0 }}" />
